...
import { EMPLOYMENT_TYPE_OPTIONS, GENDER_OPTIONS } from '../../utils/constants';
...
export default function CreateEmployeeForm({ onSuccess, onCancel }) {
  ...
  const handleSubmit = async (event) => {
    ...
    setSubmitting(true);
    try {
      const { message } = await employeeService.createEmployee({
        ...form,
        basicSalary: Number(form.basicSalary),
      });
      setNotice(message || 'Employee created. Credentials emailed to the employee.');
      onSuccess();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      ...
      <FormSelect
        ...
        options={EMPLOYMENT_TYPE_OPTIONS.map((value) => ({ value, label: formatLabel(value) }))}
      />
      <FormField
        id="basicSalary"
        label="Basic salary"
